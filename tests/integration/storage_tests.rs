...
use choreo::{Event, FunctionRun, RunStatus, StepRun, StepStatus};
use serde_json::json;
use uuid::Uuid;
...
async fn test_memory_store_get_run_by_event() {
    ...
    store
        ...
        .unwrap();
    store
        .insert_run(&FunctionRun::new("func2".to_string(), event_id, json!({})))
        .await
        .unwrap();
...
async fn test_memory_store_complete_run() {
    ...
    store
        .complete_run(run.id, json!({"result": "success"}))
        .await
        .unwrap();
...
async fn test_memory_store_fail_run() {
    ...
    store
        .fail_run(run.id, "something went wrong", false)
        .await
        .unwrap();
...
async fn test_memory_store_extend_run_lock() {
    let store = MemoryStore::new();
    let result = store
        .extend_run_lock(Uuid::new_v4(), "worker-1", 300)
        .await
        .unwrap();
    assert!(result);
}
...
async fn test_memory_store_release_run_lock() {
    let store = MemoryStore::new();
    store
        .release_run_lock(Uuid::new_v4(), "worker-1")
        .await
        .unwrap();
}
...
async fn test_memory_store_get_events_with_pagination() {
    ...
    let first_batch = store
        .get_events_by_name("paginated.event", 5, 0)
        .await
        .unwrap();
    assert_eq!(first_batch.len(), 5);

    let second_batch = store
        .get_events_by_name("paginated.event", 5, 5)
        .await
        .unwrap();
...
async fn test_memory_store_get_runs_by_function_pagination() {
    ...
    let first_batch = store
        .get_runs_by_function("paginated-func", 5, 0)
        .await
        .unwrap();
    assert_eq!(first_batch.len(), 5);

    let second_batch = store
        .get_runs_by_function("paginated-func", 5, 5)
        .await
        .unwrap();
    assert_eq!(second_batch.len(), 5);
}
