...
mod sqlite_tests {
    ...
    async fn test_event_crud() {
        ...
        let retrieved: (String, String) = sqlx::query_as(
            "SELECT name, data FROM events WHERE id = ?"
        )
        .bind(event.id.to_string())
        .fetch_one(&pool)
        .await
    ...
    async fn test_run_crud() {
        ...
        let retrieved: (String, i32) = sqlx::query_as(
            "SELECT function_id, attempt FROM runs WHERE id = ?"
        )
        .bind(run.id.to_string())
        .fetch_one(&pool)
        .await
    ...
    async fn test_step_operations() {
        ...
        sqlx::query("INSERT INTO steps (id, run_id, step_id, status, input) VALUES (?, ?, ?, ?, ?)")
    ...
    async fn test_get_events_by_name() {
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM events WHERE name LIKE 'test.event.%'")
            .fetch_one(&pool)
            .await
            .unwrap();
