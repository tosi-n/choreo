...
async fn test_step_error_serialization() {
    let err = StepError::from(serde_json::Error::io(std::io::Error::new(
...
async fn test_step_executes_when_no_cache() {
    ...
    let result: Result<serde_json::Value, StepError> = ctx
        .run("new-step", || async { Ok(json!({"executed": true})) })
        .await;
...
async fn test_step_with_timeout_success() {
    ...
    let result: Result<serde_json::Value, StepError> = ctx
        .run_with_timeout("timeout-step", Duration::from_secs(5), || async {
...
async fn test_step_with_timeout_exceeds() {
    ...
    let result: Result<serde_json::Value, StepError> = ctx
        .run_with_timeout("slow-step", Duration::from_millis(50), || async {
